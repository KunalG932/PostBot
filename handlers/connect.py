...
from main import db, router  # Importing db from main.py
...
async def cmd_connect(message: types.Message):
    ...
    try:
        ...
        if not chat_info.permissions.can_invite_users:
            await message.reply(
                "Bot must be an admin in the channel to connect. Please promote the bot and try again."
            )
            return
    except types.ChatNotFound:
        await message.reply(
            "Channel not found. Please make sure the channel exists and the bot has access to it."
        )
        return
    ...
    await message.reply(
        f"You have successfully connected to the channel: {channel_username}"
    )
...
async def cmd_connected(message: types.Message):
    ...
    if user_info and "connected_channel" in user_info:
        ...
        await message.reply(
            f"You are currently connected to the channel: {connected_channel}"
        )
    else:
        await message.reply(
            "You are not currently connected to any channel. Use /connect to connect to a channel."
        )
